...
def category_revenue(orders: List[Order]):
...
def unique_products(orders: List[Order]):
    return {product for _, product, _, _ in orders}


def customers_by_category(orders: List[Order], category_name: str):
    return sorted({name for name, _, _, category in orders if category == category_name})


def top_n_customers(customer_spending: Dict[str, float], n: int = 3):
